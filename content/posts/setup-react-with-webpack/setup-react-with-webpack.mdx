...
Install the following packages: `babel-loader` (for hooking up babel compilation into webpack), `@babel/core` (the core babel package),
`@babel/preset-env` and `@babel/preset-react` (2 presets - one for react and one for the latest JS features).

Next we'll update the webpack config to look like this:

<CodeSnippet codeString={webpackConfigCode} language="javascript" />

Here we are giving webpack a set of rules on how to handle `.jsx` and `.js` files. We want webpack
to use the `babel-loader` to compile our JS files and we've specified to use the 2 presets we've installed
to do it.
...
You'll likely want to import CSS (or SASS). For that, check out my [other tutorial](/blog/basic-setup-for-a-web-application), or have a look at some [webpack loaders](https://webpack.js.org/loaders/) on their site.